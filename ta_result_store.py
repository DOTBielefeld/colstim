...
class TargetAlgorithmObserver:
    ...
    def get_intermediate_single(self, conf_id, instance_id):
        ...
        logging.info(f"sending iter {conf_id} {instance_id} {result}")
        return result

    def get_intermediate_single_non_dominated(self, conf_id, instance_id):
        result = False
        if conf_id in list(self.results.keys()):
            if instance_id in list(self.results[conf_id].keys()):
                result = self.intermediate_output[conf_id][instance_id]
                # todo get only non dominated results of a singel?
        return result
    ...
    def get_results_single(self, conf_id, instance_id):
        ...
        logging.info(f"sending result {conf_id} {instance_id} {result}")
        return result
    ...
    def get_start_single(self, conf_id, instance_id):
        ...
        logging.info(f"sending start {conf_id} {instance_id} {result}")
        return result
    ...
    def save_trajectory(self):
        with open(f"./selector/logs/{self.scenario.log_folder}/run_history_trajectory.json", 'a') as f:
