...
class COLSTIM:
    def __init__(self, scenario, seed, features, pool_size=15, alpha=0.1, gamma=1, random_prob=0.1, mutation_prob=0.1,
        ...
        self.M = np.diag(np.ones(self.context_dim))
    ...
    def F(self, u_i, u_j):
        ...
        if u_i_e != 0 and u_j_e != 0:
            return u_i_e/(u_i_e + u_j_e)
        else:
            return 0

    def compute_gradient(self, theta, winner_id, tried_conf, instance_id):
        """
        Compute the gradient for a given feedback
        """

        u_i = np.dot(self.feature_store[winner_id][instance_id], theta)
        u_j = np.dot(self.feature_store[tried_conf][instance_id], theta)

        return (1 - self.F(u_i, u_j)) * \
               (self.feature_store[winner_id][instance_id] - self.feature_store[tried_conf][instance_id])


    def update_m(self, winner_id, tried_confs, instance_id):
        x_sum = np.zeros((self.context_dim, self.context_dim))

        for c in tried_confs:
            x_sum = x_sum + np.outer((self.feature_store[winner_id][instance_id] - self.feature_store[c][instance_id]),
                                     (self.feature_store[winner_id][instance_id] - self.feature_store[c][instance_id]))
        self.M = self.M + x_sum
    ...
    def update_model_single_observation(self, winner_id, tried_confs, instance_id):
        ...
        """
        grad_sum = np.zeros(self.context_dim)
        for c in tried_confs:
            if c != winner_id:
                grad_sum = grad_sum + self.compute_gradient(self.theta_hat, winner_id, c, instance_id)

        self.theta_hat = self.theta_hat + self.gamma * self.t ** (-self.alpha) * grad_sum
    ...
    def select_from_set(self, conf_set, instance_set, n_to_select):
        ...
        for c in range(len(conf_set)):
            ...
            for next_instance in range(len(instance_set)):
                ...
                v_hat[c][next_instance] =  np.inner(self.feature_store[conf.id][instance_set[next_instance]], self.theta_hat)
                c_hat[c][next_instance] = epsilon * np.sqrt(np.inner(np.inner(self.feature_store[conf.id][instance_set[next_instance]], M_inv),
                                                                self.feature_store[conf.id][instance_set[next_instance]]))

        v_hat_s = v_hat.sum(axis=1)
        c_hat_s = c_hat.sum(axis=1)

        quality = v_hat_s + c_hat_s
        selection = (-quality).argsort()[:n_to_select]
        print(f"choosing {selection} {(-quality).argsort()} {[conf_set[i].id for i in selection]}")
        print(f" {[[v_hat_s[i], c_hat_s[i]] for i in (-quality).argsort() ]}{list((-quality).argsort())}")

        return [conf_set[i] for i in selection], [[v_hat_s[i], c_hat_s[i]] for i in (-quality).argsort() ]
    ...
    def delete_from_pool(self, instance_set):
        ...
        for c in range(self.pool_size):
            ...
            for next_instance in range(len(instance_set)):
                v_hat[c][next_instance] =  np.inner(self.feature_store[conf.id][instance_set[next_instance]], self.theta_hat)
                c_hat[c][next_instance] = np.sqrt(np.inner(np.inner(self.feature_store[conf.id][instance_set[next_instance]], M_inv),
                                                                self.feature_store[conf.id][instance_set[next_instance]]))
        ...
        if self.dp:
            ...
            for c in range(self.pool_size):
                for oc in range(self.pool_size):
                    if c != oc and v_hat_s[oc] - c_hat_s[oc] > v_hat_s[c] + c_hat_s[c]:
    ...
    def update(self, results, previous_tournament, instance_features=None):
        ...
        for instance in previous_tournament.instance_set:
            ...
            self.update_m(best_conf_on_instance, tried, instance)
