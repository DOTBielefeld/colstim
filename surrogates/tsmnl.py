...
class TSMNL:
    def __init__(self, scenario, seed,features, pool_size=15, alpha=1, gamma=0.1, sigma=0.1, random_prob=0.2, mutation_prob=0.8,
        ...
        self.theta = copy.copy(self.theta_hat)
    ...
    def update_m(self, winner_id, tried_confs, instance_id):
        ...
        for c in tried_confs:
            x_sum = x_sum + np.outer(self.feature_store[c][instance_id],self.feature_store[c][instance_id])
        self.M = self.M + x_sum
    ...
    def update_model_single_observation(self, winner_id, tried_confs, instance_id):
        ...
        self.theta = ((self.t - 1) * (self.theta)) / self.t + (self.theta_hat / self.t)

        self.theta_bar = np.random.multivariate_normal(self.theta, self.sigma ** 2 * M_inv)
    ...
    def select_from_set(self, conf_set, instance_set, n_to_select):
        ...
        for c in range(len(conf_set)):
            ...
            for next_instance in range(len(instance_set)):
                ...
                v_hat[c][next_instance] =  np.inner(self.feature_store[conf.id][instance_set[next_instance]], self.theta_bar)
